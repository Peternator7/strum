...
#[derive(Debug, EnumProperty)]
enum TestGet {
    #[strum(props(weight = "42", flat = "true"))]
    A,
...
#[test]
fn get_int_test() {
    const BOOL_KEY: &str = "weight";
    assert_eq!(Some(42), TestGet::A.get_int(BOOL_KEY));
    assert_eq!(Some(-42), TestGet::B.get_int(BOOL_KEY));
    assert_eq!(None, TestGet::C.get_int(BOOL_KEY));
